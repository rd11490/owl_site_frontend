...
export class WinRatesPageComponent implements OnInit, OnDestroy {
  ...
  selectedRanks: string[] = ['Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond', 'Master', 'Grandmaster'];
  ...
  getLatestRate(data: WinRateData, type: 'win' | 'pick'): number {
    ...
    const sortedData = [...data.data].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
